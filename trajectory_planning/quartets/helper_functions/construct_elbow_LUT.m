function elbow_LUT = construct_elbow_LUT(q_extreme_to_staging_arr,z_vals_extreme_to_staging)
%CONSTRUCT_ELBOW_LUT Summary of this function goes here
%   Detailed explanation goes here

assert(numel(z_vals_extreme_to_staging)==1001);
...
% Construct elbow LUT
elbow_LUT = zeros(numel(z_vals_extreme_to_staging)+1, 7);
elbow_LUT(1,:) = q_staging(1:7);
elbow_LUT(2:end, 1) = 0:(numel(z_vals_extreme_to_staging)-1);
elbow_LUT(2:end, 2) = flip(z_vals_extreme_to_staging);
