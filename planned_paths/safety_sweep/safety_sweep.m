...
for j = 1:2
    all_s{end+1} = struct("q", params.q_home);
...
plotJointMotion(panda_sc, s.q, env, params)
...
for i = 1:size(paths,1)-1
    fname = pwd_name+"/temp/"+sprintf("%03d", i) + "_"+sprintf("%03d", i+1)+".mat";
...
fname_list = ["/home/oconnorlab/Code/libfranka/MATLAB/shelf_design/temp/005_007.mat"];

parfor i = 1:numel(fname_list)

    fname = fname_list(i);
    fname_char = fname{1};
    A = str2num(fname_char(end-10:end-8));
    B = str2num(fname_char(end-6:end-4));
...
%% Link together files in temp to make all_traj
...
s = struct;
s.all_traj = all_traj;
assert(motionCheck(panda_ec, panda_sc, env, s, params))

writeToCSV("safety_sweep230904", params, s)
%
%
