...
% 
% % Compute face normals for smoother shading
% normals = faceNormal(triangulation(faces, vertices));
% 
% % Plot using patch
% figure;
% c = [78 165 96]/255;
...
%           'SpecularExponent', 25, 'BackFaceLighting', 'lit');
% 
% % Improve the appearance
% axis equal;
% view(3); % Set a 3D view
% % camlight headlight; % Use a headlight for better illumination
% camlight;
% camlight('right');  % First light from the right
% camlight('left');   % Second light from the left
% material shiny; % Apply a shiny material
% title('Improved STL Rendering');
...
g = show(panda_ec_with_quartet, params.q_home, 'Frames', 'off');

% for i = 1:numel(g.Children)
%     obj = g.Children(i);
% 
%     % Check if the object is a patch (STL surface)
%     if isa(obj, 'matlab.graphics.primitive.Patch')
%         disp(['Patch ', num2str(i), ' has ', num2str(size(obj.Vertices, 1)), ' vertices.']);
%     end
% end
...
alpha_shades = linspace(0.5,1, num_shades);
for i = 1:num_frames
